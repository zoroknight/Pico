...
void OnKey(GLFWwindow* Window, int Key, int, int Action, int)
{
    ...
    Context->InputSystem->SetKeyState(
        TranslateKey(Key), Action == GLFW_PRESS);
...
int main(int Argc, char** Argv)
{
    ...
    Pico::FSceneViewportRenderer Renderer;
    ...
    try
    {
            ...
            && !GameEngine.ShouldExit())
        {
            ...
            {
                Renderer.Resize(
                    static_cast<Pico::uint32>(Width),
                    static_cast<Pico::uint32>(Height));
                Pico::FSceneView View;
                Pico::TryBuildActiveCameraView(
