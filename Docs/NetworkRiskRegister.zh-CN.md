...
学习型 MVP 网络范围已经关闭：全量 Debug 自动化 19/19，CharacterMovement 32/32；固定种子 36,000 帧回归覆盖
150 ms RTT 对应的 75 ms 单程快照延迟、约 5% 快照丢失、有界队列/平滑偏移/快照历史和最终收敛。真实三进程
10 分钟长稳测试改为发布或公网验证前的可选加固，不阻塞第 7 月。移动基座、完整代理状态、Root Motion/动态刚体
预测和生产级带宽调度仍作为后续明确风险保留。
2026-08-22 同一次 Development Stage 已通过两台真实 Windows 电脑的局域网服务器加双客户端验收，基础 LAN
可分发性风险关闭。公网路由、防火墙部署、协议版本握手、短线重连和独立 Server Target 继续由第 7 月承担。
...
- NET-R07：本月学习型 MVP 范围已关闭。输入序号、DeltaTime、ControlYaw、策略 Hash、Ack 对应状态比较和未确认
  输入重演已落地；首次三进程验收发现复制 Pawn 指针未触发客户端 Possess，导致策略 Hash 为 0、移动被拒绝且
  动画停在 Falling。现已增加 `OnRep_Pawn` 双向关系修复、晚到 Actor 的客户端绑定补偿，以及 36,000 帧
  150 ms RTT/约 5% 快照丢失快速压力回归。真实长稳和复杂移动状态仍在发布前复查。
...
当前状态：本月范围已关闭。方向、Role、Ownership、可靠性、参数类型和每连接每帧 32 次上限已落地；自动化
测试覆盖合法调用、非拥有者、可靠性不匹配和超频零副作用，一个服务器加两个客户端的开门流程已完成人工验收。
