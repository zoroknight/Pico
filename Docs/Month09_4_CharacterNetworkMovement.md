...
2026-08-22 使用同一次 Windows Development Stage 在两台真实 Windows 电脑上完成验收。电脑 A 运行可视化
独立服务器和客户端 1，电脑 B 通过电脑 A 的局域网 IPv4 与 UDP 17777 运行客户端 2；两条 Connection 均保持
Open，双向移动、跳跃和 `F` Gameplay RPC 能在服务器与另一客户端观察到一致结果。该结果补齐了自动化、
本机三进程和真实局域网三层证据，但不等同于公网、跨版本兼容或长时间发布级稳定性验证。
