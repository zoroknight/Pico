...
网络核心收尾
 ...
 -> AG1 目标读回已接入；AG2 证据范围已接入、逐项核对待补（AG4 同步评测）
 -> 资产理解与安全创作 S4～S6（待实施）
 -> AG3 房间计划编辑器入口已接入，通用 Producer 接线与在线验收待补（先于阶段 A 第 5～8 周）
...
上图是目标信任边界，不是当前部署现状。当前内置 AI Chat 的 ReAct Runtime 运行在 PicoEditor 进程内的工作任务中；
`PicoAgentHost` 目前是使用 Fake Provider 的文件协议测试宿主，不是生产 Agent 进程。外部 Codex 经本地 MCP
接入同一 Editor 工具执行服务，但其 Agent 循环由外部客户端负责。生产进程隔离及“Host 崩溃不带崩 Editor”
仍是未实现的目标；先记录真实故障与隔离需求，再决定迁移时机，不为了图示一致性重写 Runtime。
...
ReAct R5
 ...
 -> AG1 目标绑定的修改读回与后置条件（离线回归通过，Editor 在线待验收）
 -> AG2 用户要求与证据边界核对（证据范围已接入，逐项核对与在线验收待补）
 -> S4 Preview Artifacts
 -> S5 Optional Vision Provider
 -> S6 Editor Golden Tasks
 -> AG3 Build Plan 房间创作入口（首个受控场景已接入，通用 Producer 与在线验收待补）
 -> 阶段 A 第 5～8 周
```

AG4 真实任务评测从 AG1 开始持续执行，不作为最后才补的单独关卡；AG1/AG2 不撤销已放行 S4 的历史决议，
但新修改工具和新创作流程必须先满足其验证契约。
...
### Agent 执行验证与创作入口加固（AG1～AG4，实施中）

本组是对现有 ReAct、Tool Pipeline、Task State 和 Build Plan 的局部接线与验收，不新建 Harness、Memory Service
或规划模式。H2 已解决发送时编辑器快照与失败观察误绑定的问题；AG1/AG2 补的是尚未由通用门槛证明的目标级
读回和逐项回答边界，不能把 H2 工程完成状态倒写为未完成。S4 的只读预览本身无需修改门槛，但本轮排期仍
先完成 AG1/AG2；S4/S5/S6 涉及的新
修改工具与阶段 A 游戏创作入口，必须复用 AG1 的执行验证契约。

| 节点 | 实施范围 | 验收与停止条件 |
| --- | --- | --- |
| AG1：目标绑定读回（优先） | 修改结果记录稳定目标、Revision/版本及可验证后置条件；Runtime 的待验证状态绑定该目标。只接受对应目标的新鲜读回或工具验证器的后置条件证据，不让无关只读查询解除待验证状态；沿用现有事务、审批、Operation Journal 和 Undo。 | 修改 A 后读 B 仍待验证；读 A 且后置条件满足才可完成；目标变化、Revision 冲突、读回失败、取消或恢复后不误报成功。离线覆盖无关读回、同名不同目标、重试与重启恢复，Editor 中验证批准/拒绝/Undo。 |
| AG2：回答证据边界 | 在现有 Task State/Observation 中按用户要求记录已验证、未验证、不可判断及证据来源/Revision；缺项时仅做必要的定向补查，预算耗尽则明确报告边界。先覆盖资产引用与内部参数、材质可见特征、World/Selection 当前状态、房间封闭性，不新增逐句审稿模型。 | 只读到资产引用不能声称验证了内部材质参数或视觉外观；Actor 列表不能单独证明房间封闭；切换 World/Selection 后旧结论不冒充当前事实。允许不同的合理回答深度和只读工具顺序。 |
| AG3：Build Plan 创作入口 | 将已有 Requirement/Spec、能力诊断、Build Plan/DryRun 与 PlanHash 接入内置 AI Chat，在临时项目先做一种小而完整的受控场景：需求 -> 差异预览 -> 绑定计划的一次审批 -> 事务执行 -> AG1 定向读回 -> Undo。复用现有 Producer 和工具安全边界；不开放任意代码/文件生成。 | 用户可在 Editor 中完成整条流程；拒绝零副作用；批准后结果与预览一致；计划或 World 变化使旧审批失效；失败回滚、保存重开和 Undo 可核验。库层单测不代替产品入口验收。 |
| AG4：真实任务与成本评测（贯穿 AG1～AG3） | 复用 H1 任务集、采集脚本与 Trace，补齐真实 Provider/Editor 复核；按任务要求覆盖、证据、最终状态和副作用判定，工具顺序只作诊断。另记录未命中 Token、总输入、延迟、工具数和 FPS；S6 仍承担更完整的端到端 Editor Golden Tasks。 | 同任务允许多种有效工具路径；安全项零退化，质量不低于 H1 基线；性能/成本与质量一起报告。样本不足时不宣称稳定 P95、因果缓存收益或全场景成功率。 |

实施顺序为 AG1 -> AG2 -> AG3，AG4 每步同步采集和复核。AG1/AG2 预计约一周，AG3 约一至两周；
若 H1 真实任务暴露检索或 Skill 问题，再按 H4 单独处理。暂不引入向量数据库、多 Agent、第二套记忆、
固定工具 DAG 或统一回答篇幅；生产进程隔离保留为独立安全决策，不混入本轮局部加固。

当前实现状态：AG1 的目标绑定待验证状态、定向读回及工具后置条件验证已有离线回归；AG2 已给 Observation
补证据范围并强化提示，但尚无按用户逐项要求执行的确定性核对器；AG3 仅将 Build Plan/DryRun/PlanHash
接入 `editor.scene.preview_room_plan` 和 `editor.scene.apply_room_plan` 这一个房间壳场景，复用现有房间工具、
审批和 Undo，不代表通用 Producer 工作流已接入。AG1～AG3 的真实 Provider/Editor 验收与 AG4 质量、
成本、FPS 复核均未完成。执行步骤和证据记录见 [AG 验收清单](AgentAGValidation.zh-CN.md)。

长对话房间计划复测暴露了跨轮状态边界：旧 ToolResult 超过历史投影的单项事实预算时，PlanHash 被省略，
下一轮模型无法仅凭压缩历史继续。现已在既有 ToolResult/Session/进度上下文中接入**有界续接提示**：
工具声明操作类别、只读取回工具和 open/closed 状态；Compact 开启时 Runtime 仅从最近事件提取最多两条
提示，不携带旧 PlanHash。仅当用户确实继续该操作时才调用取回工具，World/目标有效性与审批仍由工具链
重新核验；成功执行会关闭提示。该接线不扩大聊天窗口，也不引入第二套记忆服务。离线回归已覆盖大结果
省略、拒绝后保留、成功后关闭、畸形输入和过期窗口；真实 Provider 的长历史续接/重启/成本复测仍待执行。
